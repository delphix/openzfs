...
usage(void)
{
	...
	(void) fprintf(stderr, "        --bucket <bucket> object-store "
	    "bucket\n");
...
dump_metaslab_groups(spa_t *spa, boolean_t show_special)
{
	...
	/*
	 * Allocations from the log class are skipped because they
	 * belong to the ZIL. Allocations from the object store class
	 * are also skipped as we don't use real metaslabs for object
	 * store back-ends.
	 */
...
zdb_leak_init(spa_t *spa, zdb_cb_t *zcb)
{
	 ...
	 * use the tree.
	 */
	spa->spa_object_store_class->mc_ops = &zdb_metaslab_ops;
...
dump_block_stats(spa_t *spa)
{
	...
	if (spa_is_object_based(spa)) {
		zcb->zcb_totalasize +=
		    metaslab_class_get_alloc(spa_object_store_class(spa));
	}
...
dump_agent_metadata(spa_t *spa)
{
	...
	if (err != 0) {
		(void) printf("Could not get pool phys from object agent: %u\n",
		    err);
		return;
	}
	...
	err = libzoa_get_uberblock_phys(zoa_handle, spa_guid(spa), last_txg,
	    &uberblock_phys);
	if (err != 0) {
		(void) printf("Could not get uberblock from object agent: %u\n",
		    err);
		return;
	}
	...
	err = libzoa_find_leaks(zoa_handle, &leaked_objects);
	if (err != 0) {
		(void) printf("Could not get leaks from object agent: %u\n",
		    err);
		return;
	}
	uint_t leaks_len;
	char **leaks = fnvlist_lookup_string_array(leaked_objects,
	    "leaked", &leaks_len);
	if (leaks_len > 0) {
		(void) printf("%u Leaked object store objects:\n", leaks_len);
		for (uint_t i = 0; i < leaks_len; i++) {
			printf("\t%s\n", leaks[i]);
		}
		printf("\n");
	}
	...
	uint_t missing_len;
	char **missing = fnvlist_lookup_string_array(leaked_objects,
	    "missing", &missing_len);
	if (missing_len > 0) {
		(void) printf("%u Missing object store objects:\n",
		    missing_len);
		for (uint_t i = 0; i < missing_len; i++) {
			printf("\t%s\n", missing[i]);
		}
		printf("\n");
	}
...
main(int argc, char **argv)
{
	...
	struct option long_options[] = {
		...
		{"bucket",		required_argument,	NULL, 1},
		{0, 0, 0, 0}
	};

	while ((c = getopt_long(argc, argv,
	    "a:ABbcCdDeEf:Fg:GhiI:kK:lLmMNo:Op:pPqrRsSt:T:uU:vVx:XYyZz:",
	    long_options, NULL)) != -1) {
		switch (c) {
		case 'b':
		case 'B':
...
#else
		(void) fprintf(stderr, "Object store operations not valid "
		    "without libzoa support");
		exit(1);
#endif
	}
#if defined(_LP64)
...
#else
		fatal("cannot open '%s' in agent: no libzoa support "
		    "configured");
#endif
	}
