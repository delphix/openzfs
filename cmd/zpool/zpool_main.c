...
zpool_do_create(int argc, char **argv)
{
	...
	/*
	 * We do not persist the object store properties as zpool properties,
	 * so remove them now that they have been consumed.
	 */
    ...
    nvlist_t *nv, int depth, boolean_t isspare, vdev_rebuild_stat_t *vrs)
{
	...
	if (!isspare) {
		...
		if (cb->cb_print_rate_limit) {
			if (children == 0 && strcmp(type, VDEV_TYPE_OBJSTORE) ==
			    0)  {
				/*
				 * Only object store vdevs have rate limit
				 * errors.
				 */
				zfs_nicenum(vs->vs_rate_limit_errors, rbuf,
				    sizeof (rbuf));
			} else {
				snprintf(rbuf, sizeof (rbuf), "-");
			}
			...
			if (cb->cb_literal) {
				printf(" %5llu",
				    (u_longlong_t)vs->vs_rate_limit_errors);
			} else {
				printf(" %5s", rbuf);
			}
		}
...
status_callback(zpool_handle_t *zhp, void *data)
{
	...
	case ZPOOL_STATUS_ERRATA:
		...
		break;
	case ZPOOL_STATUS_RATE_LIMITED:
		printf_color(ANSI_BOLD, gettext("status: "));
		printf_color(ANSI_YELLOW, gettext("The pool's backend object "
		    "store returned one or more rate limit errors.\n\t"
		    "No data was lost as a result, but performance may be "
		    "negatively affected.\n"));
		printf_color(ANSI_BOLD, gettext("action: "));
		printf_color(ANSI_YELLOW, gettext("Examine the object agent "
		    "logs for details about the rate limit error,\n\t"
		    "consult the object store backend's documentation, and "
	...
	if (config != NULL) {
		...
		if (cbp->cb_print_rate_limit) {
			printf_color(ANSI_BOLD, " %5s", gettext("RATE"));
		}
...
zpool_do_status(int argc, char **argv)
{
	...
	/* check options */
	while ((c = getopt_long(argc, argv, "c:igLpPsvxDtT:dwr", long_options,
	    NULL)) != -1) {
		switch (c) {
		case 'c':
			if (cmd != NULL) {
...
upgrade_enable_all(zpool_handle_t *zhp, int *countp)
{
	...
	for (i = 0; i < SPA_FEATURES; i++) {
		...
		if (!spa_feature_table[i].fi_zfs_mod_supported ||
		    ((spa_feature_table[i].fi_flags & ZFEATURE_FLAG_AGENT) &&
