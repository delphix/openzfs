...
{
	zs->zs_alloc = metaslab_class_get_alloc(spa_default_class(ztest_spa));
	zs->zs_space = metaslab_class_get_space(spa_default_class(ztest_spa));
...
ztest_run_zdb(const char *pool, uint64_t guid)
{
	...
	if (ztest_opts.zo_obj_store) {
		ASSERT3P(guid, !=, 0);
		snprintf(loc, len, "-T %s %s %s %s %s --bucket %s %s %s %llu",
		    ztest_opts.zo_obj_store_protocol,
...
ztest_freeze(void)
{
	 ...
	 * capacity.
	 */
	metaslab_class_t *default_class = spa_default_class(spa);
	uint64_t capacity = metaslab_class_get_space(default_class) / 2;
	...
	while (ztest_random(10) != 0 &&
	    ...
	    metaslab_class_get_alloc(default_class) < capacity) {
		ztest_od_t od;
...
ztest_run(ztest_shared_t *zs)
{
	...
	zs->zs_alloc = metaslab_class_get_alloc(spa_default_class(spa));
	zs->zs_space = metaslab_class_get_space(spa_default_class(spa));
