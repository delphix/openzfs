...
nvlist_t **
fnvlist_lookup_nvlist_array(nvlist_t *nvl, const char *name, uint_t *n)
{
	nvlist_t **rv;
	VERIFY0(nvlist_lookup_nvlist_array(nvl, name, &rv, n));
	return (rv);
}

char **
fnvlist_lookup_string_array(nvlist_t *nvl, const char *name, uint_t *n)
{
	char **rv;
	VERIFY0(nvlist_lookup_string_array(nvl, name, &rv, n));
	return (rv);
}
...
EXPORT_SYMBOL(fnvlist_lookup_nvlist_array);
EXPORT_SYMBOL(fnvlist_lookup_string_array);
