...
zfs_ioc_recv_impl(char *tofs, char *tosnap, const char *origin,
    ...
    boolean_t force, boolean_t heal, boolean_t resumable, boolean_t illumos,
    int input_fd, dmu_replay_record_t *begin_record, uint64_t *read_bytes,
    uint64_t *errflags, nvlist_t **errors)
{
	...
	*errors = fnvlist_alloc();
	...
	error = dmu_recv_begin(tofs, tosnap, begin_record, force,
	    heal, resumable, illumos, localprops, hidden_args, origin, &drc,
	    input_fp, &off);
	if (error != 0)
		goto out;
...
zfs_ioc_recv_new(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	*tosnap++ = '\0';
	...
	error = zfs_ioc_recv_impl(tofs, tosnap, origin, recvprops, localprops,
	    hidden_args, force, heal, resumable, illumos, input_fd,
