    ...
    void (*func)(uint64_t, vdev_t *, uint64_t, uint64_t, void *), void *arg)
{
	...
	for (remap_segment_t *rs = rs_alloc(vd, offset, asize, 0);
		...
		for (uint64_t i = 0; i < num_entries; i++) {
			...
			if (vdev_is_object_store(dst_v)) {
				/*
				 * Refer to `vdev_mapping_sync_object_store()`
				 * and `spa_vdev_copy_segment_object_store()`
				 * for more context on why we break our segments
				 * to source_vdev ashift chunks.
				 */
				uint64_t sector_size = 1 << v->vdev_ashift;
				ASSERT0(P2PHASE(inner_size, sector_size));

				uint64_t target_blockid =
				    DVA_GET_BLOCKID(&m->vimep_dst) +
				    (inner_offset >> v->vdev_ashift);
				for (uint64_t block_offset = 0;
				    block_offset < inner_size;
				    block_offset += sector_size,
				    target_blockid++) {
					func(rs->rs_split_offset + block_offset,
					    dst_v,
					    target_blockid << SPA_MINBLOCKSHIFT,
					    sector_size, arg);
				}
			} else if ((zfs_flags & ZFS_DEBUG_INDIRECT_REMAP) &&
...
vdev_indirect_io_start(zio_t *zio)
{
	...
	if (first->is_size == zio->io_size) {
		...
		zio_nowait(zio_vdev_child_io(zio, zio->io_bp,
		    ...
		    abd_get_offset_size(zio->io_abd, 0,
		    zio->io_size), zio->io_size, zio->io_type,
		    zio->io_priority, 0,
