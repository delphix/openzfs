...
{
	int allocators = (ops == &zfs_objectstore_ops) ?
	    1 : spa->spa_alloc_count;

	metaslab_class_t *mc = kmem_zalloc(offsetof(metaslab_class_t,
	    mc_allocator[allocators]), KM_SLEEP);
...
{
	int allocators = metaslab_class_allocator_count(mc);
...
metaslab_class_histogram_verify(metaslab_class_t *mc)
{
	...
	for (int c = 0; c < rvd->vdev_children; c++) {
		 ...
		 * vdevs that are not in this metaslab class.
		 */
...
int
metaslab_class_allocator_count(metaslab_class_t *mc)
{
	return ((mc == spa_object_store_class(mc->mc_spa)) ?
	    1 : mc->mc_spa->spa_alloc_count);
}
...
metaslab_group_activate(metaslab_group_t *mg)
{
	...
	for (int i = 0; i < metaslab_group_allocator_count(mg); i++) {
...
int
metaslab_group_allocator_count(metaslab_group_t *mg)
{
	return (metaslab_class_allocator_count(mg->mg_class));
}
...
metaslab_load(metaslab_t *msp)
{
	 ...
	 * zfs_metaslab_mem_limit tunable. For any other class (e.g. object
	 * store, log, etc..) we don't care.
	 */
...
metaslab_sync_done(metaslab_t *msp, uint64_t txg)
{
	...
	uint64_t free_space = metaslab_class_get_space(spa_default_class(spa)) -
	    metaslab_class_get_alloc(spa_default_class(spa));
...
static uint64_t
metaslab_group_alloc_object(metaslab_group_t *mg,
    int allocator, uint64_t blocks)
{
	metaslab_class_t *mc = mg->mg_class;
	...
	uint64_t first_blockid = mc->mc_ops->msop_alloc(msp, blocks);
	if (zfs_flags & ZFS_DEBUG_OBJECT_STORE) {
		zfs_dbgmsg("ALLOC: start: %llu, run: %llu, next %llu",
		    (u_longlong_t)first_blockid,
		    (u_longlong_t)blocks,
		    (u_longlong_t)msp->ms_lbas[0]);
	}
	VERIFY3U(first_blockid, !=, -1ULL);
	mutex_exit(&msp->ms_lock);

	mutex_enter(&mg->mg_lock);
	mg->mg_allocations += blocks;
	mutex_exit(&mg->mg_lock);

	return (first_blockid);
}
...
{
	ASSERT(mg->mg_initialized);
	ASSERT(!vdev_is_object_based(mg->mg_vd));

	uint64_t offset = metaslab_group_alloc_normal(mg, zal, asize, txg,
...
/*
 * Allocate multiple small blocks (1 << ashift) from the object-store vdev
 * to make up the given `psize`. The allocated object-store block IDs are
 * sequential and in order (n, n+1, ...). The `dva` parameter will use the
 * block ID of the first allocated block as its block ID and the sum of block
 * sizes for its ASIZE.
 *
 * These "multiblock" DVAs are special-purpose and currently only used for
 * indirect mappings that map segments from removed block devices to the
 * object-store. It is not possible to free the first 1KB out of a 2KB object
 * store block (you have to free the whole object for any space to be freed).
 * Similarly you can't just read that first 1KB without fetching the whole 2KB
 * object and this can hurt throughput/bandwidth. Breaking bigger segments from
 * block devices down to the minimum sized block in the object store gives the
 * benefit of granularity when it comes to reads and frees of sub-ranges in
 * such segment mappings.
 *
 * Note that these multiblock DVAs are indistinguishable from standard
 * single-block DVAs and thus should only be used as destination DVAs for
 * indirect mappings pointing to the object store. If we ever decide to use
 * them in other places, potentially mixing them with single-block DVAs, we
 * would need to make multiblock DVAs self-identifying (e.g., by setting one
 * of the grid bits in the DVA).
 */
int
metaslab_alloc_object_multiblock_dva(spa_t *spa,
    uint64_t ashift, uint64_t psize, dva_t *dva)
{
	spa_config_enter(spa, SCL_ALLOC, FTAG, RW_READER);

	vdev_t *vd = vdev_find_leaf(spa->spa_root_vdev, &vdev_object_store_ops);
	uint64_t asize = vdev_psize_to_asize(vd, psize);

	ASSERT0(P2PHASE(asize, 1 << ashift));
	uint64_t nblocks = asize >> ashift;

	uint64_t first_blockid =
	    metaslab_group_alloc_object(vd->vdev_mg, 0, nblocks);
	ASSERT3U(first_blockid, !=, -1ULL);

	DVA_SET_BLOCKID(&dva[0], first_blockid);
...
top:
	do {
		...
		if (vdev_is_object_based(mg->mg_vd)) {
			offset = metaslab_group_alloc_object(mg,
			    allocator, 1);
			ASSERT3U(offset, !=, -1ULL);
		} else {
			/*
			 * If we don't need to try hard, then require
			 * that the block be on a different metaslab
			 * from any other DVAs in this BP (unique=true).
			 * If we are trying hard, then allow any metaslab
			 * to be used (unique=false).
			 */
    ...
    uint64_t size, void *arg)
{
	...
	/*
	 * Indirect mappings that point to the object store use multi-block
	 * DVAs as their destination which are currently indistinguishable
	 * from standard single-block DVAs. For this reason, multi-block
	 * DVAs are not applicable outside the context of indirect mappings
	 * and thus should not be used for anything else, so we skip them
	 * here.
	 */
	if (vdev_is_object_store(vd))
		return;
