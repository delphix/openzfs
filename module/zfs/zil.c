...
zil_sums_init(zil_sums_t *zs)
{
	...
	wmsum_init(&zs->zil_slog_alloc_failures, 0);
	wmsum_init(&zs->zil_skip_zil_commit, 0);
}
...
zil_sums_fini(zil_sums_t *zs)
{
	...
	wmsum_fini(&zs->zil_slog_alloc_failures);
	wmsum_fini(&zs->zil_skip_zil_commit);
}
...
zil_kstat_values_update(zil_kstat_values_t *zs, zil_sums_t *zil_sums)
{
	...
	zs->zil_slog_alloc_failures.value.ui64 =
	    wmsum_value(&zil_sums->zil_slog_alloc_failures);
	zs->zil_skip_zil_commit.value.ui64 =
	    wmsum_value(&zil_sums->zil_skip_zil_commit);
}
...
zil_create(zilog_t *zilog)
{
	...
	if (BP_IS_HOLE(&blk) || BP_SHOULD_BYTESWAP(&blk)) {
		...
		if (error == 0)
			...
			ZIL_STAT_BUMP(zilog, zil_slog_alloc_failures);
	}
...
zil_commit(zilog_t *zilog, uint64_t foid)
{
	...
	/*
	 * If this is an object store pool and we have no slog devices skip
	 * committing this zil. Note, in a hybrid pool (e.g. backed by both
	 * an object store and block devices) we could fall back to
	 * allocating the zil from the normal/embedded_log class if there
	 * are no slog devices. This functionality is left as potential
	 * future work.
	 */
	if (spa_is_object_based(zilog->zl_spa) &&
	    !spa_has_slogs(zilog->zl_spa)) {
		ZIL_STAT_BUMP(zilog, zil_skip_zil_commit);
		return;
	}
