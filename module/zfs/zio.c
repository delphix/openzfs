...
zio_add_child(zio_t *pio, zio_t *cio)
{
	 ...
	 */
	if (spa_is_object_based(pio->io_spa) &&
...
zio_free(spa_t *spa, uint64_t txg, const blkptr_t *bp)
{
	...
	if (BP_IS_GANG(bp) ||
	    ...
	    !spa_is_object_based(spa)) || brt_maybe_exists(spa, bp)) {
...
zio_trim(zio_t *pio, vdev_t *vd, uint64_t offset, uint64_t size,
    zio_done_func_t *done, void *private, zio_priority_t priority,
    zio_flag_t flags, enum zio_control_flag zio_control_flags)
{
	zio_t *zio;
...
{
	/*
	 * Object store vdevs have their own mechanism for flushing
	 * their changes to the backend - see `object_store_end_txg()`.
	 */
    ...
    uint64_t offset)
{
	...
	uint64_t gb_size = BP_GET_PSIZE(bp);
	return (zio_read(pio, pio->io_spa, bp,
	    abd_get_offset_size(data, offset, gb_size), gb_size,
...
{
	spa_t *spa = zio->io_spa;
	metaslab_class_t *mc = spa_object_store_class(spa);
	blkptr_t *bp = zio->io_bp;
	int flags = 0;
    ...
    uint64_t size, boolean_t *slog)
{
	...
	ASSERT3U(spa->spa_alloc_count, ==,
	    metaslab_class_allocator_count(spa_log_class(spa)));
	ASSERT3U(spa->spa_alloc_count, ==,
	    metaslab_class_allocator_count(spa_embedded_log_class(spa)));
	ASSERT3U(spa->spa_alloc_count, ==,
	    metaslab_class_allocator_count(spa_normal_class(spa)));
...
zio_ready(zio_t *zio)
{
	...
	if (spa_is_object_based(zio->io_spa) &&
