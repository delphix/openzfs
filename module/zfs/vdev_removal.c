...
vdev_passivate(vdev_t *vd, uint64_t *txg)
{
	...
	if (mg->mg_class == normal) {
		...
		/*
		 * We can't remove all storage. There must be at least one
		 * disk (or an object store vdev) left in the pool.
		 */
		if (last && !spa_is_object_based(spa))
...
spa_vdev_noalloc(spa_t *spa, uint64_t guid)
{
	...
	else if (vdev_is_object_based(vd))
		error = SET_ERROR(ZFS_ERR_VDEV_NOTSUP);
...
static void
spa_vdev_removal_copy_arg_init(vdev_copy_arg_t *vca)
{
...
spa_vdev_removal_create(vdev_t *vd)
{
	...
	spa_vdev_removal_copy_arg_init(&svr->svr_vca);
...
static void
spa_vdev_removal_copy_arg_fini(vdev_copy_arg_t *vca)
{
...
{
	spa_vdev_removal_copy_arg_fini(&svr->svr_vca);
...
static void
object_store_mapping_write_done(zio_t *zio)
{
	spa_vdev_removal_t *svr = zio->io_private;
	mutex_enter(&svr->svr_vca.vca_lock);
	svr->svr_vca.vca_outstanding_bytes -= zio->io_size;
	if (zio->io_error != 0) {
		svr->svr_vca.vca_write_error_bytes +=
		    zio->io_size;
	}
	cv_signal(&svr->svr_vca.vca_cv);
	mutex_exit(&svr->svr_vca.vca_lock);
	abd_free(zio->io_abd);
}

static void
object_store_mapping_free_after_write_done(zio_t *zio)
{
	object_store_mapping_write_done(zio);
	ASSERT(vdev_is_object_store(zio->io_vd));
	metaslab_free_concrete(zio->io_vd,
	    zio->io_offset, zio->io_size, B_FALSE);
}

static void
object_store_mapping_parent_done(zio_t *zio)
{
	vdev_indirect_mapping_entry_t *entry = zio->io_private;
	entry->vime_obsolete_count +=
	    range_tree_space(entry->vime_obsolete_segs);
	range_tree_vacate(entry->vime_obsolete_segs, NULL, NULL);
	range_tree_destroy(entry->vime_obsolete_segs);
	abd_free(entry->vime_data);
}

static void
vdev_mapping_sync_object_store(spa_vdev_removal_t *svr, dmu_tx_t *tx)
{
	spa_t *spa = dmu_tx_pool(tx)->dp_spa;
	...
	vdev_t *removing_vdev = vdev_lookup_top(spa, svr->svr_vdev_id);

	zio_t *rio = zio_root(spa, NULL, NULL, 0);

	/*
	 * Since we are doing the allocations outside of the zio
	 * pipeline context below, we need to make sure that all
	 * the writes are flushed to the object store. Thus, we
	 * set the following io_control flag to flush all the
	 * writes to the object store.
	 */
	rio->io_control_flags |= ZIO_CONTROL_FLUSH_WRITES;

	spa_config_enter(spa, SCL_ZIO, FTAG, RW_READER);

	list_t *list = &svr->svr_new_segments[dmu_tx_get_txg(tx) & TXG_MASK];
	for (vdev_indirect_mapping_entry_t *entry = list_head(list); entry;
	    entry = list_next(list, entry)) {
		vdev_t *dst_vd = vdev_lookup_top(spa,
		    DVA_GET_VDEV(&entry->vime_mapping.vimep_dst));

		/* skip indirect mappings whose destination is a normal vdev */
		if (!vdev_is_object_based(dst_vd))
			continue;

		uint64_t size =
		    DVA_GET_ASIZE(&entry->vime_mapping.vimep_dst);
		uint64_t sector_size = 1 << removing_vdev->vdev_ashift;

		VERIFY0(metaslab_alloc_object_multiblock_dva(spa,
		    removing_vdev->vdev_ashift, size,
		    &entry->vime_mapping.vimep_dst));

		zio_t *pio = zio_null(rio, spa, NULL,
		    object_store_mapping_parent_done, entry, 0);
		dva_t *dst = &entry->vime_mapping.vimep_dst;
		uint64_t start_blockid = DVA_GET_BLOCKID(dst);

		/*
		 * io_max_offset's below set the agent's max_blkid
		 * which is used to keep track of how many blocks
		 * we've allocated in the object store. Update it
		 * here since we just allocated new blocks from the
		 * object store.
		 */
		uint64_t num_blockids =
		    size >> removing_vdev->vdev_ashift;
		uint64_t max_blockid =
		    start_blockid + num_blockids - 1;
		mutex_enter(&pio->io_lock);
		pio->io_max_offset = MAX(pio->io_max_offset,
		    max_blockid << SPA_MINBLOCKSHIFT);
		mutex_exit(&pio->io_lock);
		mutex_enter(&rio->io_lock);
		rio->io_max_offset = MAX(rio->io_max_offset,
		    max_blockid << SPA_MINBLOCKSHIFT);
		mutex_exit(&rio->io_lock);

		uint64_t blockid = start_blockid;
		for (uint64_t abd_offset = 0;
		    abd_offset < size;
		    abd_offset += sector_size, blockid++) {

			/*
			 * If this is an obsolete segment make sure to free it
			 * right after the write.
			 */
			boolean_t free_after_write =
			    range_tree_contains(entry->vime_obsolete_segs,
			    abd_offset, sector_size);

			zio_t *write_zio = zio_vdev_child_io(pio, NULL,
			    dst_vd, blockid << SPA_MINBLOCKSHIFT,
			    abd_get_offset_size(entry->vime_data,
			    abd_offset, sector_size), sector_size,
			    ZIO_TYPE_WRITE, ZIO_PRIORITY_REMOVAL,
			    ZIO_FLAG_CANFAIL, (free_after_write) ?
			    object_store_mapping_free_after_write_done :
			    object_store_mapping_write_done, svr);

			mutex_enter(&write_zio->io_lock);
			write_zio->io_max_offset =
			    blockid << SPA_MINBLOCKSHIFT;
			mutex_exit(&write_zio->io_lock);

			zio_nowait(write_zio);
		}
		zio_nowait(pio);
	}
	zio_wait(rio);
 ...
 * Called as a sync task in every txg that the removal thread makes
 * progress. If this is a hybrid pool the function also performs
 * writes for any data being evacuated from a removing block device
 * to the object store.
 */
static void
vdev_mapping_sync(void *arg, dmu_tx_t *tx)
{
	...
	vdev_mapping_sync_object_store(svr, tx);
...
/*
 * Given the segment ranges and `maxalloc` set `start` and `size` defining
 * the range that's next in line to be evacuated from the removing vdev.
 */
static void
spa_vdev_removal_next_segment_range(range_tree_t *segs, uint64_t maxalloc,
    uint64_t *start, uint64_t *size)
{
	*start = range_tree_min(segs);
	*size = range_tree_span(segs);
...
/*
 * Return a range tree containing obsolete subranges within a segment
 * specified by `start` and `size`. The offsets of the ranges in the
 * returned tree are relative to the `start` offset passed as an argument
 * to the function.
 *
 * Note: The returned tree was allocated from memory and should be freed
 * once it's no longer needed.
 */
static range_tree_t *
spa_vdev_removal_obsolete_subranges(range_tree_t *segs,
    uint64_t start, uint64_t size)
{
	/*
	...
	/* We can't end in the middle of an obsolete range */
	ASSERT3U(start + size, <=, prev_seg_end);

	return (obsolete_segs);
}

static void
spa_vdev_copy_segment_object_store_done(zio_t *zio)
{
	vdev_copy_segment_arg_t *vcsa = zio->io_private;

	ASSERT3P(vcsa->vcsa_obsolete_segs, ==, NULL);
...
static void
spa_vdev_copy_segment_object_store_read_done(zio_t *zio)
{
...
/*
 * Object store allocations and writes can only happen in syncing context.
 * Because of that, this function creates the zio to read the segment from
 * the evacuating vdev and temporarily copies its data to a buffer in the
 * indirect mapping that it creates. The write to the object store happens
 * later in syncing context when we sync the indirect mappings for the
 * current TXG (after which point the aforementioned buffer is freed). See
 * `vdev_mapping_sync_object_store()` for more information.
 */
static int
spa_vdev_copy_segment_object_store(vdev_t *vd, range_tree_t *segs,
    uint64_t maxalloc, uint64_t txg, vdev_copy_arg_t *vca)
{
	spa_t *spa = vd->vdev_spa;
	...
	uint64_t start, size;
	spa_vdev_removal_next_segment_range(segs, maxalloc, &start, &size);
	ASSERT0(P2PHASE(start, 1 << spa->spa_min_ashift));
	ASSERT0(P2PHASE(size, 1 << spa->spa_min_ashift));

	range_tree_t *obsolete_segs =
	    spa_vdev_removal_obsolete_subranges(segs, start, size);

	range_tree_clear(segs, start, size);

	/*
	 * Even though we don't allocate any space for this DVA yet we:
	 *
	 * [1] Set is destination vdev to be the object store. This is
	 *   important because it allows us to know whether we should do
	 *   any object-store allocations when syncing the mappings later
	 *   or not.
	 *
	 * [2] Set the DVA's size now because the source block in the
	 *   indirect mapping doesn't have a record of it and we need it.
	 */
	dva_t dst = {{ 0 }};
	vdev_t *object_store_vdev =
	    vdev_find_leaf(spa->spa_root_vdev, &vdev_object_store_ops);
	DVA_SET_VDEV(&dst, object_store_vdev->vdev_id);
	DVA_SET_ASIZE(&dst, size);
	ASSERT(vdev_writeable(object_store_vdev));

	vdev_indirect_mapping_entry_t *entry =
	    kmem_zalloc(sizeof (vdev_indirect_mapping_entry_t), KM_SLEEP);
	DVA_MAPPING_SET_SRC_OFFSET(&entry->vime_mapping, start);
	entry->vime_mapping.vimep_dst = dst;
	if (spa_feature_is_enabled(spa, SPA_FEATURE_OBSOLETE_COUNTS)) {
		entry->vime_obsolete_segs = obsolete_segs;
	}
	...
	/*
	 * The spa_config_lock must be held while our read zio is in
	 * progress, to ensure that the vdev tree does not change (e.g.
	 * due to a concurrent "zpool attach/detach"). The "null" zio
	 * is needed to release the spa_config_lock after the read zio
	 * completes.
	 */
	spa_config_enter(spa, SCL_STATE, spa, RW_READER);
	zio_t *nzio = zio_null(spa->spa_txg_zio[txg & TXG_MASK], spa, NULL,
	    spa_vdev_copy_segment_object_store_done, vcsa, 0);

	entry->vime_data = abd_alloc_for_io(size, B_FALSE);
	mutex_enter(&vca->vca_lock);
	vca->vca_outstanding_bytes += size;
	mutex_exit(&vca->vca_lock);

	vdev_t *source_vd = NULL;
	if (vd->vdev_ops == &vdev_mirror_ops) {
		/* If this is a mirror always read from a readable child. */
		for (int i = 0; i < vd->vdev_children; i++) {
			source_vd = vd->vdev_child[i];
			if (vdev_readable(source_vd))
				break;
		}
	} else {
		source_vd = vd;
	 ...
	 * the pool would be in a suspended state.
	 */
	VERIFY3P(source_vd, !=, NULL);

	zio_nowait(zio_vdev_child_io(nzio, NULL,
	    source_vd, start, entry->vime_data, size,
	    ZIO_TYPE_READ, ZIO_PRIORITY_REMOVAL,
	    ZIO_FLAG_CANFAIL,
	    spa_vdev_copy_segment_object_store_read_done, vca));
	zio_nowait(nzio);
 ...
 */
static void
spa_vdev_copy_impl(vdev_t *vd, spa_vdev_removal_t *svr, uint64_t *max_alloc,
    dmu_tx_t *tx)
{
	uint64_t txg = dmu_tx_get_txg(tx);
	spa_t *spa = dmu_tx_pool(tx)->dp_spa;
	...
	while (!range_tree_is_empty(segs)) {
		...
		/*
		 * If this is a hybrid pool (i.e. contains both block
		 * and object store vdevs) then all removal allocations
		 * will end up in the object store vdev. Note that in
		 * this current design this means that the data of an
		 * evacuating special-class vdev will end up in the
		 * object-store and not in any other special-class vdevs
		 * that may be present in the pool.
		 *
		 * Also note that a pool can become hybrid in the middle
		 * of a device removal causing some allocations from this
		 * thread to end up in a normal-class vdev and the rest
		 * in the object store vdev.
		 */
		if (spa_is_object_based(spa)) {
			error = spa_vdev_copy_segment_object_store(vd,
			    segs, thismax, txg, &svr->svr_vca);
		} else {
			error = spa_vdev_copy_segment(vd,
			    segs, thismax, txg, &svr->svr_vca, &zal);
		}
...
spa_vdev_remove_thread(void *arg)
{
	...
	for (msi = start_offset >> vd->vdev_ms_shift;
		...
		while (!svr->svr_thread_exit &&
			...
			spa_vdev_copy_impl(vd, svr, &max_alloc, tx);
		...
		if (zfs_removal_ignore_errors == 0 &&
		    (svr->svr_vca.vca_read_error_bytes > 0 ||
		    svr->svr_vca.vca_write_error_bytes > 0)) {
	 ...
	 */
	txg_wait_synced(spa->spa_dsl_pool, 0);
	ASSERT0(svr->svr_vca.vca_outstanding_bytes);
	...
	if (svr->svr_thread_exit) {
		...
		if (zfs_removal_ignore_errors == 0 &&
			...
			zfs_dbgmsg("canceling removal due to IO errors: "
			    ...
			    (u_longlong_t)svr->svr_vca.vca_read_error_bytes,
			    (u_longlong_t)svr->svr_vca.vca_write_error_bytes);
...
spa_vdev_remove_top_check(vdev_t *vd)
{
	 ...
	 * in a block-based pool. If this is a hybrid pool we
	 * don't care about differences in ashift as the data
	 * from the removal will always flow to the object
	 * store which defaults to ashift 9 and uses multiblock
	 * DVAs.
	 */
	if (!spa_is_object_based(spa) &&
	    spa->spa_max_ashift != spa->spa_min_ashift) {
		return (SET_ERROR(EINVAL));
	}
...
/* BEGIN CSTYLED */
ZFS_MODULE_PARAM(zfs_vdev, zfs_, remove_max_copy_bytes, UINT, ZMOD_RW,
