...
dsl_process_async_destroys(dsl_pool_t *dp, dmu_tx_t *tx)
{
	...
	if (dp->dp_free_dir != NULL && !scn->scn_async_destroying &&
		...
		dsl_dir_phys_t *phys = dsl_dir_phys(dp->dp_free_dir);
		uint64_t used = phys->dd_used_bytes;
		uint64_t comp = phys->dd_compressed_bytes;
		uint64_t uncomp = phys->dd_uncompressed_bytes;
		if (used != 0) {
			spa_history_log_internal(spa, "async destroy", tx,
			    "ERROR: completed but dp_free_dir->dd_used_bytes "
			    "is nonzero: %lld (comp=%lld uncomp=%lld)",
			    (long long)used,
			    (long long)comp,
			    (long long)uncomp);
		}
