...
vdev_metaslab_group_create(vdev_t *vd)
{
	...
	if (vd->vdev_mg == NULL) {
		...
		metaslab_class_t *default_class = vdev_is_object_based(vd) ?
		...
		vd->vdev_mg = metaslab_group_create(mc, vd,
		    metaslab_class_allocator_count(mc));
		IMPLY(vdev_is_object_based(vd),
		    vd->vdev_mg->mg_allocators == 1);
...
vdev_metaslab_init(vdev_t *vd, uint64_t txg)
{
	 ...
	 * group.
	 *
	 * Only do so if vdev is part of the normal class. We don't allow
	 * ZIL allocation for object-based vdevs.
	 */
...
vdev_clear(spa_t *spa, vdev_t *vd)
{
	...
	vd->vdev_stat.vs_rate_limit_errors = 0;
...
/*
 * This predicate is similar to `vdev_is_object_based()` but only checks the
 * vdev parameter passed to it and not any children under it. It also doesn't
 * attempt to grab any config locks.
 */
boolean_t
vdev_is_object_store(vdev_t *vd)
{
	return ((vd != NULL) ?
	    (vd->vdev_ops == &vdev_object_store_ops) : B_FALSE);
}
