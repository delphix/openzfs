...
dsl_pool_unreserved_space(dsl_pool_t *dp, zfs_space_check_t slop_policy)
{
	...
	uint64_t deferred = dsl_pool_deferred_space(dp);
...
{
	spa_t *spa = dp->dp_spa;
	return (metaslab_class_get_deferred(spa_normal_class(spa))
	    + metaslab_class_get_deferred(spa_object_store_class(spa)));
}
