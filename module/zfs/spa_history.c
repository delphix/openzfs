...
spa_history_create_obj(spa_t *spa, dmu_tx_t *tx)
{
	...
	/*
	 * Figure out maximum size of history log. This is 1G for object
	 * store pools, because they have unbounded size, and 0.1% of
	 * pool size (max 1G) for all other pools.
	 */
	if (spa_is_object_based(spa)) {
		shpp->sh_phys_max_off = 1<<30;
	} else {
