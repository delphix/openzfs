...
spa_config_update(spa_t *spa, int what)
{
	...
	if (what == SPA_CONFIG_UPDATE_VDEVS) {
		/*
		 * If we recently added an object store vdev we need
		 * to reset the pool's type. That needs to happen at
		 * a point when that vdev is allocatable so we do it
		 * here after creating its metaslab metadata above
		 * and waiting for one TXG for them to sync to disk.
		 */
