...
static const zio_taskq_info_t zio_taskqs[ZIO_TYPES][ZIO_TASKQ_TYPES] = {
	...
	{ ZTI_SCALE,	ZTI_N(5),	ZTI_SCALE,	ZTI_N(5) }, /* WRITE */
...
spa_prop_get_config(spa_t *spa, nvlist_t **nvp)
{
	...
	metaslab_class_t *mc = spa_default_class(spa);

	ASSERT(MUTEX_HELD(&spa->spa_props_lock));

	if (rvd != NULL) {
		alloc = metaslab_class_get_alloc(spa_normal_class(spa));
		alloc += metaslab_class_get_alloc(spa_object_store_class(spa));
		alloc += metaslab_class_get_alloc(spa_special_class(spa));
		alloc += metaslab_class_get_alloc(spa_dedup_class(spa));
		alloc += metaslab_class_get_alloc(spa_embedded_log_class(spa));

		size = metaslab_class_get_space(spa_normal_class(spa));
		size += metaslab_class_get_space(spa_object_store_class(spa));
	...
	if (pool != NULL) {
		...
		if (pool->dp_free_dir != NULL) {
			...
			/*
			 * Due to an undiagnosed bug, the free_dir's used may go
			 * negative.  Rectify it here so that the property
			 * doesn't have a bogus value of >2^63.
			 */
			if ((int64_t)freeing < 0) {
				freeing = 0;
			}
		}
...
/*
 * If this pool is object-based, then properties related to the object-store
 * are part of the root vdev. Extract any properties that are found and add
 * them to the zpool properties.
 */
static void
spa_get_object_store_props(spa_t *spa, nvlist_t **nvp)
{
	...
	if (vd != NULL) {
		...
		nv = fnvlist_alloc();
		vd->vdev_ops->vdev_op_config_generate(vd, nv, B_FALSE);

		if (!nvlist_lookup_string(nv,
		    zpool_prop_to_name(ZPOOL_PROP_OBJ_PROTOCOL), &val)) {
			spa_prop_add_list(*nvp, ZPOOL_PROP_OBJ_PROTOCOL, val,
			    0, ZPROP_SRC_LOCAL);
		}
		if (!nvlist_lookup_string(nv,
		    zpool_prop_to_name(ZPOOL_PROP_OBJ_ENDPOINT), &val)) {
			spa_prop_add_list(*nvp, ZPOOL_PROP_OBJ_ENDPOINT, val,
			    0, ZPROP_SRC_LOCAL);
		}
		if (!nvlist_lookup_string(nv,
		    zpool_prop_to_name(ZPOOL_PROP_OBJ_REGION), &val)) {
			spa_prop_add_list(*nvp, ZPOOL_PROP_OBJ_REGION, val, 0,
			    ZPROP_SRC_LOCAL);
		}
		if (!nvlist_lookup_string(nv,
		    zpool_prop_to_name(ZPOOL_PROP_OBJ_CRED_PROFILE), &val)) {
			spa_prop_add_list(*nvp, ZPOOL_PROP_OBJ_CRED_PROFILE,
			    val, 0, ZPROP_SRC_LOCAL);
		}
	}
}
...
spa_prop_get(spa_t *spa, nvlist_t **nvp)
{
	...
	spa_get_object_store_props(spa, nvp);
...
spa_activate(spa_t *spa, spa_mode_t mode)
{
	...
	spa->spa_object_store_class =
	    metaslab_class_create(spa, &zfs_objectstore_ops);
...
spa_deactivate(spa_t *spa)
{
	...
	metaslab_class_destroy(spa->spa_object_store_class);
	spa->spa_object_store_class = NULL;
...
spa_sync_adjust_vdev_max_queue_depth(spa_t *spa)
{
	...
	metaslab_class_t *object_store = spa_object_store_class(spa);
	...
	/*
	 * We have effectively unlimited queue depth for the object store
	 * so we disable the dva throttle.
	 */
	object_store->mc_allocator[0].mca_alloc_max_slots = slots_per_allocator;
	ASSERT0(zfs_refcount_count(
	    &object_store->mc_allocator[0].mca_alloc_slots));
	object_store->mc_alloc_throttle_enabled = B_FALSE;
}
...
spa_sync(spa_t *spa, uint64_t txg)
{
	...
	metaslab_class_evict_old(spa->spa_object_store_class, txg);
