...
/*
 * Set the pool's type based on whether the config contains an
 * object store vdev or not. It does not check whether the object
 * store vdev is allocatable. This makes this wrapper suitable
 * only for the scenario of pool creation where there is no
 * allocation metadata (e.g metaslabs) initialized yet for any
 * device in the pool.
 *
 * To set the pool's type during import or device addition use
 * `spa_set_pool_type_from_vdev_metadata()` instead.
 */
void
spa_set_pool_type_from_vdev_tree(spa_t *spa)
{
	...
	spa->spa_pool_type = (vdev_is_object_based(spa->spa_root_vdev)) ?
	    SPA_TYPE_OBJECT_STORE : SPA_TYPE_NORMAL;
}

/*
 * Set the pool's type based on whether the config contains an
 * object store vdev AND whether that vdev is allocatable or not.
 */
void
spa_set_pool_type_from_vdev_metadata(spa_t *spa)
{
	ASSERT3P(spa->spa_root_vdev, !=, NULL);
	...
	/*
	 * Even if there is an object store vdev, it may not have a
	 * metaslab array (i.e. it doesn't accept allocations yet).
	 */
	if (object_store == NULL || object_store->vdev_ms_array == 0)
...
void
spa_access_block(spa_t *spa, const dva_t *dva)
{
	if (spa_is_object_based(spa)) {
		spa_config_enter(spa, SCL_VDEV, FTAG, RW_READER);
		vdev_t *vd = vdev_lookup_top(spa, DVA_GET_VDEV(dva));
		if (vdev_is_object_based(vd)) {
			object_store_prefetch_block(vd, DVA_GET_OFFSET(dva));
		}
    ...
    const char *tag)
{
	 ...
	 */
	ASSERT(metaslab_class_validate(spa_object_store_class(spa)) == 0);
...
spa_update_dspace(spa_t *spa)
{
	spa->spa_dspace = metaslab_class_get_dspace(spa_normal_class(spa)) +
	    metaslab_class_get_dspace(spa_object_store_class(spa)) +
...
metaslab_class_t *
spa_default_class(spa_t *spa)
{
	return (spa_is_object_based(spa) ?
	    spa_object_store_class(spa) : spa_normal_class(spa));
}

metaslab_class_t *
spa_object_store_class(spa_t *spa)
{
	return (spa->spa_object_store_class);
}
