    ...
    uberblock_t *ub, vdev_t *vd, int flags)
{
	...
	/*
	 * Object store uberblock is written in `object_store_end_txg()`.
	 */
...
/*
 * Sync the uberblocks to the object store vdev (if it exists)
 * and all vdevs in svd[].
 */
static int
vdev_uberblock_sync_list(vdev_t **svd, int svdcount, uberblock_t *ub, int flags)
{
	...
	/*
	 * If this pool is using object storage, then we need to notify the
	 * backend that we've completed the txg. This operation combines
	 * syncing the uberblock/labels/config to the object store. We
	 * always do this first as it's the object store's config that we'll
	 * use to open the pool.
	 */
	if (spa_is_object_based(spa)) {
		vdev_t *vd_os = vdev_find_leaf(spa->spa_root_vdev,
		    &vdev_object_store_ops);
		nvlist_t *label = spa_config_generate(spa,
		    vd_os, ub->ub_txg, B_FALSE);
		object_store_end_txg(vd_os, label, ub->ub_txg);
		good_writes++;
	}
...
vdev_label_sync_list(spa_t *spa, int l, uint64_t txg, int flags)
{
	...
	for (vdev_t *vd = list_head(dl); vd != NULL;
	    vd = list_next(dl, vd)) {
		ASSERT(!vd->vdev_ishole);

		/*
		 * Object store label is updated
		 * in `object_store_end_txg()`.
		 */
		if (vdev_is_object_store(vd))
			continue;
	...
	for (vdev_t *vd = list_head(dl); vd != NULL; vd = list_next(dl, vd))
		zio_flush(zio, vd);
...
retry:
	...
	/*
	 * Object-store pools may go past the spa_final_dirty_txg
	 * (see comment in ub_dp_mos_used_delta for more).
	 */
	if (!spa_is_object_based(spa)) {
	...
	/*
	 * Sync the uberblocks to the object store vdev (if it exists) and
	 * all the vdevs in svd[]. If the system dies in the middle of this
	 * step, there are two cases to consider, and the on-disk state is
	...
	if ((error = vdev_uberblock_sync_list(svd, svdcount, ub, flags)) != 0) {
		if ((flags & ZIO_FLAG_TRYHARD) != 0) {
			zfs_dbgmsg("vdev_uberblock_sync_list() "
			    "returned error %d for pool '%s'",
			    error, spa_name(spa));
		}
		goto retry;
	}
