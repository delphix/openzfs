...
/*
 * Number of seconds before notifying the agent of a subsequent access.
 * This should be shorter than the time it takes for blocks to fall out of
 * the zettacache.
 */
static int zfs_arc_agent_access_delay = 10 * 60;
...
static boolean_t arc_access(arc_buf_hdr_t *, arc_flags_t, boolean_t);
...
arc_hdr_realloc_crypt(arc_buf_hdr_t *hdr, boolean_t need_crypt)
{
	...
	nhdr->b_l1hdr.b_agent_access = hdr->b_l1hdr.b_agent_access;
...
/*
 * This routine is called whenever a buffer is accessed.  Returns TRUE if
 * the object agent (zettacache) should be notified about this access.
 */
static boolean_t
arc_access(arc_buf_hdr_t *hdr, arc_flags_t arc_flags, boolean_t hit)
{
	...
	if (hdr->b_l1hdr.b_state == arc_anon) {
		...
		hdr->b_l1hdr.b_agent_access = now;
		if (HDR_UNCACHED(hdr)) {
	...
	if (hdr->b_l1hdr.b_agent_access == 0) {
		hdr->b_l1hdr.b_agent_access = now;
	} else if (ddi_time_after(now, hdr->b_l1hdr.b_agent_access +
	    hz * zfs_arc_agent_access_delay)) {
		hdr->b_l1hdr.b_agent_access = now;
		return (B_TRUE);
	}
	return (B_FALSE);
 ...
 */
void
arc_buf_access(arc_buf_t *buf, spa_t *spa)
{
	arc_buf_hdr_t *hdr = buf->b_hdr;
	...
	boolean_t notify_agent = arc_access(hdr, 0, B_TRUE);
	mutex_exit(hash_lock);

	if (notify_agent) {
		spa_access_block(spa, &hdr->b_dva);
	}
    ...
    int zio_flags, arc_flags_t *arc_flags, const zbookmark_phys_t *zb)
{
	...
	arc_buf_t *buf = NULL;
	int rc = 0;
	boolean_t notify_agent = B_FALSE;
...
top:
	...
	if (hdr != NULL && HDR_HAS_L1HDR(hdr) && (HDR_HAS_RABD(hdr) ||
		...
		if (HDR_IO_IN_PROGRESS(hdr)) {
			...
			notify_agent = arc_access(hdr, *arc_flags, B_FALSE);
		...
		DTRACE_PROBE1(arc__hit, arc_buf_hdr_t *, hdr);
		notify_agent = arc_access(hdr, *arc_flags, B_TRUE);
...
out:
	...
	if (notify_agent) {
		spa_access_block(spa, BP_IDENTITY(bp));
	}
	spl_fstrans_unmark(cookie);
	return (rc);
...
ZFS_MODULE_PARAM(zfs_arc, zfs_arc_, agent_access_delay, INT, ZMOD_RW,
	"Delay before notifying agent of access, in seconds");
