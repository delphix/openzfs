 ...
 *
 * Scenario [4]: For hybrid pools or any object-based pool with special
 * devices (e.g. slogs, dedup vdevs, etc..) we can have summary entries
 * with no blocks (lse_blkcount is or gets decremented to 0) but non-zero
 * mscount (similar to scenario [3]). This is due to object store pools
 * syncing TXGs but not necessarily allocating/freeing data in the metaslabs
 * of normal and special vdevs which leads to creating empty log spacemaps
 * without any blocks. It is fine to not do anything on that scenario as
 * these summary entries will eventually get freed when we flush enough
 * metaslabs and their mscount gets to 0.
 */
void
spa_log_summary_decrement_blkcount(spa_t *spa, uint64_t blocks_gone)
{
	...
	for (; e != NULL; e = list_head(&spa->spa_log_summary)) {
		if (e->lse_blkcount > blocks_gone) {
			...
			/* Verify that this is scenario [3] or [4] */
			VERIFY3U(blocks_gone, ==, e->lse_blkcount);

			if (!spa_is_object_based(spa)) {
				/*
				 * This looks like scenario [3]. Verify this
				 * by ensuring that his is the only entry in
				 * the summary.
				 */
				VERIFY3P(e, ==,
				    list_tail(&spa->spa_log_summary));
				ASSERT3P(e, ==,
