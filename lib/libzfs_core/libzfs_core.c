...
recv_impl(const char *snapname, nvlist_t *recvdprops, nvlist_t *localprops,
    ...
    boolean_t heal, boolean_t resumable, boolean_t raw, boolean_t illumos,
    int input_fd, const dmu_replay_record_t *begin_record, uint64_t *read_bytes,
    uint64_t *errflags, nvlist_t **errors)
{
	...
	*atp = '\0';
	 ...
	 */
	if (resumable || heal || raw || illumos || wkeydata != NULL ||
	    payload) {
...
{
	return (recv_impl(snapname, props, NULL, NULL, 0, origin, force,
	    B_FALSE, B_TRUE, raw, B_TRUE, fd, NULL, NULL, NULL, NULL));
}
    ...
    int fd, const dmu_replay_record_t *begin_record)
{
	...
	return (recv_impl(snapname, props, NULL, NULL, 0, origin, force,
	    B_FALSE, resumable, raw, B_FALSE, fd, begin_record, NULL, NULL,
	    NULL));
}
    ...
    nvlist_t **errors)
{
	(void) action_handle, (void) cleanup_fd;
	return (recv_impl(snapname, props, cmdprops, wkeydata, wkeylen, origin,
	    force, heal, resumable, raw, B_FALSE, input_fd, begin_record,
	    read_bytes, errflags, errors));
}
