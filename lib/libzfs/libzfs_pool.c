...
zpool_vdev_remove_wanted(zpool_handle_t *zhp, const char *path)
{
	...
	/*
	 * This function is too aggressive if there is no redundancy
	 * in the pool for the indicated device. This will cause the
	 * call to hang in the kernel and the pool to eventually
	 * suspend. Until this is resolved return failure, as we don't
	 * rely on this functionality in the Delphix product.
	 */
	if (path != NULL)
