...
static int
vdev_rate_limited(vdev_stat_t *vs, uint_t vsc)
{
	(void) vsc;
	return (vs->vs_rate_limit_errors != 0);
}
    ...
    zpool_errata_t *erratap, const char *compat)
{
	 ...
	 * Rate limited
	 */
	if (find_vdev_problem(nvroot, vdev_rate_limited, B_TRUE))
		return (ZPOOL_STATUS_RATE_LIMITED);
...
/*
 * This function is only used by the Delphix Appliance to check whether the
 * pool is rate limited without having to maintain a copy of the ZPOOL_STATUS
 * enum. We should remove this when upstreaming.
 */
boolean_t
zpool_is_rate_limited(zpool_handle_t *zhp)
{
	return (zpool_get_status(zhp, NULL, NULL) == ZPOOL_STATUS_RATE_LIMITED);
}
