    ...
    const char *version_req_str, nvlist_t **version)
{
	...
	for (;;) {
		...
		if (sock < 0) {
			...
			zutil_error(hdl, LPC_SOCKETFAILURE,
		...
		if (errno == ENOENT) {
			...
			zutil_error(hdl, LPC_CONNECT_RETRY,
			...
			zutil_error_aux(hdl, "%s", strerror(errno));
			zutil_error(hdl, LPC_CONNECT_REFUSED,
			    dgettext(TEXT_DOMAIN,
	...
	if (resp == NULL) {
		...
		zutil_error(hdl, LPC_CONNECT_REFUSED, dgettext(TEXT_DOMAIN,
    ...
    const char *version_req_str, zoa_socket_t zoa_sock, nvlist_t **version)
{
	...
	for (; retries < ZOA_MAX_RETRIES; retries++) {
		...
		if (err == 0) {
			...
			zutil_error_fmt(hdl, LPC_CONNECTION_ERROR,
			    dgettext(TEXT_DOMAIN,
			    "error communicating with object agent: %d"), err);
			break;
		}
		(void) zutil_error_fmt(hdl, LPC_CONNECTION_ERROR,
		    dgettext(TEXT_DOMAIN,
...
zoa_list_destroy_pools(libpc_handle_t *hdl, boolean_t destroy_complete)
{
	...
	while ((elem = nvlist_next_nvpair(nvpools, elem)) != NULL) {
		...
		if (nvlist_lookup_boolean_value(config,
			...
			const char *state = fnvlist_lookup_string(config,
			    AGENT_DESTROY_STATE);
			item.destroyed =
