...
libpc_error_description(libpc_handle_t *hdl)
{
	...
	case LPC_EACCESS:
		return (dgettext(TEXT_DOMAIN, "some devices require root "
		    "privileges"));
	case LPC_SOCKETFAILURE:
		return (dgettext(TEXT_DOMAIN, "socket failure"));
	case LPC_CONNECT_REFUSED:
		return (dgettext(TEXT_DOMAIN, "agent may not be running"));
	case LPC_CONNECT_RETRY:
		return (dgettext(TEXT_DOMAIN, "retrying connection"));
	case LPC_CONNECTION_ERROR:
		return (dgettext(TEXT_DOMAIN, "error communicating with "
		    "agent"));
	case LPC_UNKNOWN:
...
zpool_find_import_agent(libpc_handle_t *hdl, importargs_t *iarg,
    pthread_mutex_t *lock, avl_tree_t *cache)
{
	const char *profile = NULL, *bucket = NULL, *endpoint = NULL;
	const char *region = NULL, *protocol = NULL;
	...
	while ((elem = nvlist_next_nvpair(pools, elem)) != NULL) {
		...
		/*
		 * object-store properties are not stored in the zpool config
		 * but zpool import needs them to import the pool. So, add
