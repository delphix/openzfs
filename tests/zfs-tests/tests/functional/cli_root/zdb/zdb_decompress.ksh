...
output=$(run_zdb -q -e "-R" -p "$TESTPOOL $vdev:$offset:$size_str:d")
echo $output | grep -q $pattern || log_fail "zdb -R :d failed to decompress the data properly"

output=$(run_zdb -q -e "-R" -p "$TESTPOOL $vdev:$offset:$size_str:dr")
...
output=$(run_zdb -q -e "-R" -p "$TESTPOOL $vdev:$offset:$lsize:dr")
result=${#output}
(( $result != $blksize)) && log_fail \
