...
typedef struct vdev_indirect_mapping_entry {
	...
	/*
	 * The following two fields are only used when evacuating a
	 * removing block device's data to the object store.
	 *
	 * - `vime_data` contains the data of a segment read from the
	 *   removing device which will then be copied to a segment
	 *   allocated in the object store in syncing context. Once
	 *   copied this abd buffer is freed.
	 *
	 * - `vime_obsolete_segs` contains ranges within the
	 *   `vime_data` segment that are free and like `vime_data`
	 *   is used in syncing context to free those ranges once
	 *   the removing segment is allocated in the object store.
	 *
	 * See `vdev_mapping_sync_object_store()` for the usage of
	 * these fields.
	 */
	abd_t	*vime_data;
	range_tree_t *vime_obsolete_segs;
