...
typedef enum {
	...
	/*
	 * Encountered rate limiting erorrs from the backend object store. IOs
	 * were retried until successful, but administrator attention may be
	 * required to resolve performance issues.
	 */
	ZPOOL_STATUS_RATE_LIMITED,
...
_LIBZFS_H zpool_status_t zpool_import_status(nvlist_t *, const char **,
    zpool_errata_t *);
_LIBZFS_H boolean_t zpool_is_rate_limited(zpool_handle_t *);
